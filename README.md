...
- [3. 实证分析结果](#3-实证分析结果)
  - [3.1 基准回归结果](#31-基准回归结果)
  - [3.2 日内收益结果](#32-日内收益结果)
  - [3.3 基准回归结果可视化](#33-基准回归结果可视化)
  - [3.4 隔夜收益结果](#34-隔夜收益结果)
  - [3.5 安慰剂检验](#35-安慰剂检验)
  - [3.6 检验结果可视化](#36-检验结果可视化)
  - [3.7 安慰剂检验结果分析](#37-安慰剂检验结果分析)
  - [3.8 初步结论](#38-初步结论)
- [4. 局限性与后续分析](#4-局限性与后续分析)
  - [4.1 样本量有限](#41-样本量有限)
  - [4.2 情绪变量本身存在测量误差](#42-情绪变量本身存在测量误差)
  - [4.3 当前模型没有控制其他信息](#43-当前模型没有控制其他信息)
  - [4.4 金融数据质量与时间匹配问题](#44-金融数据质量与时间匹配问题)
  - [4.5 后续分析方向](#45-后续分析方向)
---
...
在原研究中，我们也对文本情绪做了研究，并得出了相应结论。但由于流程相近，且本复盘仅作演示，只挑选了部分数据作为对象，故不再重复该流程。
...
前面的回归结果显示，在当前样本中，面部情绪与之后的短期沪深 300 收益之间存在一定的统计关系，并且随机打乱情绪数据后的安慰剂组没有复现原始结果。

不过，这并不意味着当前研究已经证明：

> 面部情绪直接导致了股票市场的上涨或下跌。

本次分析更准确的定位应该是一次基于有限数据的独立复盘与探索。

作为第一次尝试处理分钟级金融数据并进行实证分析，我在完成整个流程后，也逐渐发现了一些当前研究中仍然存在的问题。
...
从表面上看，97 个观测值似乎已经可以进行回归分析。

但这里需要注意一个问题：

> 这些观测值并不是来自 97 场完全独立的新闻发布会。

实际上，本次复盘只使用了有限数量的央行发布会。

同一场发布会中的多个分钟可能具有相似的背景。
...
因此，同一场发布会中的第 1 分钟和第 10 分钟并不能完全被看作两个独立的实验。

这意味着：

> 实际上能够提供真正独立信息的“事件数量”，可能比分钟级观测数量更少。

Newey-West HAC 标准误能够在一定程度上处理时间序列中的异方差和自相关问题，但它并不能自动解决所有来自同一发布会内部的相关性。
...
除此之外，也可以进一步尝试按照“发布会”而不是单纯的“分钟”来考虑数据结构。

例如，可以研究：

> 一场发布会整体的平均情绪，与发布会前后市场变化之间的关系。

或者在未来尝试使用按发布会进行聚类的标准误。

---

### 4.2 情绪变量本身存在测量误差

本次研究中的情绪变量并不是直接观察到的“真实情绪”。
...
模型可能将某个自然表情识别为 Happiness，也可能因为视频角度、光线或者表情变化较小而产生识别误差。

另外，本研究中的 Face-A 和 Face-B 本身也属于人为定义的不同情绪分类方式。

例如：

* Face-A 将 Disgust 视为负面情绪；
* Face-B 则将 Disgust 排除。

这说明：

> 最终的回归结果可能受到情绪定义方式的影响。

事实上，在当前数据中，Face-A 和 Face-B 的部分回归结果非常接近。

这可能说明在当前样本中，两种分类方式没有产生明显不同的信息，也可能反映出情绪数据本身存在较高程度的相关性。
...
> 当前模型识别得到的面部情绪指标，与之后的市场收益之间存在一定统计关系。
...
如果一种结果只在某一个非常特殊的情绪定义下成立，那么它的解释需要更加谨慎。

反过来，如果不同的合理定义都能够得到类似结果，则可以增加研究结论的可信度。
...
> 面部情绪本身可能并没有直接影响市场，但面部情绪与讲话内容或者政策信息同时变化。

例如，如果发布会传递了积极的政策信号，那么：
...
> 在控制文字信息之后，面部情绪是否仍然与市场收益存在独立的统计关系？

如果控制文本之后，面部情绪仍然保持显著，那么非语言信息可能确实提供了一部分额外的信息。

如果加入文本后情绪变量的显著性消失，则可能说明：

> 原先观察到的关系主要反映了语言信息和非语言信息之间的共同变化。

---

### 4.4 金融数据质量与时间匹配问题

本次复盘使用的是分钟级沪深 300 数据。
...
对于日频数据来说，这种误差通常不会造成明显影响。

但在分钟级金融数据中：

> 几分钟甚至几十秒的偏差，都可能改变最终的收益计算结果。
...
* 进一步确认分钟数据的时间定义；
* 检查不同时间匹配方式下结果是否稳定；
* 尝试不同的收益窗口，例如 $t+3$、$t+5$、$t+10$、$t+20$。

如果仅仅改变几分钟的匹配方式，回归结果就发生巨大变化，那么说明模型可能对时间匹配非常敏感。

相反，如果不同的合理时间窗口都能观察到类似趋势，则可以提高结果的稳健性。

---

## 4.5 后续分析方向

基于以上问题，如果继续推进这个项目，我认为最有价值的方向主要有以下几个。
...
> 当前发现的关系是否只存在于少数发布会，还是能够在更长时间范围内重复出现？
...
> 在控制一种信息之后，另一种信息是否仍然能够提供额外的解释能力？
...
目前的 Placebo Test 使用了一次随机打乱。

它提供了一个初步结果：

> 随机情绪数据没有复现原始数据中的显著关系。
...
然后比较真实回归得到的 $\beta$ 位于随机分布的什么位置。

这可以回答一个更加直观的问题：

> 如果情绪和市场实际上没有关系，那么随机情况下出现当前这么大的回归系数有多罕见？

这将是对当前 Placebo Test 的一个自然延伸。
...
因此，未来如果继续研究，更重要的可能不是继续增加更多复杂的模型，而是首先明确：

> 哪一种研究设计能够帮助我们区分这些不同的解释？
...
但随着数据处理、收益计算、HAC 标准误和 Placebo Test 的逐步完成，我逐渐意识到：

> 得到一个显著的 p-value，并不是实证研究的终点。
...
因此，本仓库最终并不试图给出一个确定的结论：

> 面部情绪一定会影响股票市场。

相反，这次复盘更接近一次学习过程。

从最初处理视频情绪数据，到构造分钟级收益，再到理解 OLS、Newey-West HAC 和 Placebo Test，我逐渐完成了一次从“运行代码”到“开始理解研究方法”的过程。

当前结果确实发现了一些值得进一步研究的统计关系。

但比结果本身更重要的是：

> 任何一个看起来显著的结果，都需要继续追问它是如何产生的。
